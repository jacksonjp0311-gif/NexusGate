...
def build_discovery_cards(root: str | Path) -> dict[str, Any]:
    ...
    cards = [
        ...
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "inner-communication-without-autonomous-authority",
            "version": "0.1.0",
            "title": "Inner Communication Without Autonomous Authority",
            "status": "active",
            "summary": "NEX organs may exchange typed evidence packets while authority remains outside message reliability.",
            "math": {"reliability": "provenance * verification * freshness * confidence"},
            "code_references": ["nexus_gate/nex_core/messages.py", "nexus_gate/nex_core/bus.py"],
            "algorithm_card_refs": ["nex-inner-message-bus"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-inner-status"],
            "evidence_surfaces": ["ledger/nex_inner_messages.jsonl"],
            "next_versions": ["v0.2: add more organ-specific predictive messages"],
            "boundary": "Reliable inner messages do not create execution or authorization power.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "teaching-is-not-learning",
            "version": "0.1.0",
            "title": "Teaching Is Not Learning",
            "status": "active",
            "summary": "A teaching episode records evidence; learning remains a proposal until replay, measurement, retention, and human authorization pass.",
            "math": {"gate": "min(P,V,A,S,H,R)"},
            "code_references": ["nexus_gate/nex_core/teach.py", "nexus_gate/nex_core/learn.py"],
            "algorithm_card_refs": ["teaching-episode-seal", "geometric-learning-proposal"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-teach-begin", ".\\scripts\\nexus.ps1 nex-learn-propose"],
            "evidence_surfaces": ["ledger/nex_teaching_events.jsonl", "ledger/nex_learning_events.jsonl"],
            "next_versions": ["v0.2: bind teaching episodes to verified Experience Seals"],
            "boundary": "Teaching cannot apply persistent conductance.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "acceptance-is-not-validation",
            "version": "0.1.0",
            "title": "Acceptance Is Not Validation",
            "status": "active",
            "summary": "Human acceptance, test validation, and causal attribution are independent quality dimensions.",
            "math": {"quality": "accepted without tests => V=0"},
            "code_references": ["nexus_gate/nex_core/teach.py"],
            "algorithm_card_refs": ["teaching-episode-seal"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-teach-seal -Disposition accepted"],
            "evidence_surfaces": ["state/nex_core/teaching/"],
            "next_versions": ["v0.2: add validation bundle signatures"],
            "boundary": "Friendly disposition cannot masquerade as evidence of correctness.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "prediction-error-as-attention-pressure",
            "version": "0.1.0",
            "title": "Prediction Error As Attention Pressure",
            "status": "active",
            "summary": "Prediction error can prioritize inspection without authorizing action.",
            "math": {"priority": "|error| * uncertainty * consequence * freshness"},
            "code_references": ["nexus_gate/nex_core/build.py"],
            "algorithm_card_refs": ["prediction-error-salience"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-verify-all"],
            "evidence_surfaces": ["reports/nexus_cognitive_cycle_latest.json"],
            "next_versions": ["v0.2: feed measured prediction error into Breath pressure"],
            "boundary": "Attention is not authority.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "fast-experience-and-slow-knowledge",
            "version": "0.1.0",
            "title": "Fast Experience And Slow Knowledge",
            "status": "active",
            "summary": "NEX separates one-event episodic teaching from slow semantic consolidation that requires repeated verified support.",
            "math": {"repetition": "R = min(1, independent_verified / 3)"},
            "code_references": ["nexus_gate/nex_core/teach.py", "nexus_gate/nex_core/learn.py"],
            "algorithm_card_refs": ["fast-episodic-teaching", "slow-semantic-consolidation"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-learn-status"],
            "evidence_surfaces": ["state/nex_core/teaching/", "state/nex_core/learning/"],
            "next_versions": ["v0.2: add consolidation windows"],
            "boundary": "A single episode may inform but cannot persistently influence conductance.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "convergent-graph-activation",
            "version": "0.1.0",
            "title": "Convergent Graph Activation",
            "status": "active",
            "summary": "Damped internal activation uses a contraction so bounded cycles do not self-amplify indefinitely.",
            "math": {"update": "a(k+1)=(1-d)s+dPa(k), d=0.82"},
            "code_references": ["nexus_gate/nex_core/build.py"],
            "algorithm_card_refs": ["damped-cognitive-diffusion"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-chat -Tag \"What is pending?\""],
            "evidence_surfaces": ["reports/nexus_nex_core_latest.json"],
            "next_versions": ["v0.2: add larger cognitive subgraphs"],
            "boundary": "Convergence does not imply consciousness.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "small-local-weight-changes-preserve-stability",
            "version": "0.1.0",
            "title": "Small Local Weight Changes Preserve Stability",
            "status": "active",
            "summary": "Log-conductance proposals cap single updates so lessons nudge routing rather than violently rewiring it.",
            "math": {"delta": "clip(eta*Q*u - penalties, +/-0.05)"},
            "code_references": ["nexus_gate/nex_core/learn.py"],
            "algorithm_card_refs": ["log-conductance-update"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-learn-propose"],
            "evidence_surfaces": ["ledger/nex_learning_events.jsonl"],
            "next_versions": ["v0.2: add authorized application after repeated verified teachings"],
            "boundary": "A weight cannot authorize itself.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "structured-rationale-without-chain-of-thought-storage",
            "version": "0.1.0",
            "title": "Structured Rationale Without Chain-Of-Thought Storage",
            "status": "active",
            "summary": "NEX CORE exposes selected intent, evidence paths, contradictions, uncertainty, and blocked claims without storing hidden free-form reasoning.",
            "math": {"trace": "evidence paths + blocked claims"},
            "code_references": ["nexus_gate/nex_core/chat.py", "electron/renderer/renderer.js"],
            "algorithm_card_refs": ["grounded-nex-core-response"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-chat -Tag \"Explain your boundary\""],
            "evidence_surfaces": ["reports/nexus_nex_core_latest.json"],
            "next_versions": ["v0.2: add collapsible UI trace filters"],
            "boundary": "Auditable evidence trace is allowed; hidden chain-of-thought storage is not required.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "direct-nglm-conversation-without-frontier-model",
            "version": "0.1.0",
            "title": "Direct NGLM Conversation Without A Frontier Model",
            "status": "active",
            "summary": "NEX CORE routes chat directly to local NGLM and verified evidence, bypassing Ollama and neural-model bridge paths.",
            "math": {"route": "NEX_CORE -> nexus:askNexCore -> nexus_gate.nex_core.cli chat"},
            "code_references": ["electron/main.js", "electron/preload.js", "electron/renderer/renderer.js", "nexus_gate/nex_core/chat.py"],
            "algorithm_card_refs": ["cognitive-cycle-coordinator"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-mode-status", ".\\scripts\\nexus.ps1 nex-chat -Tag \"What can you claim?\""],
            "evidence_surfaces": ["reports/nexus_nex_core_latest.json"],
            "next_versions": ["v0.2: add richer UI trace panels"],
            "boundary": "No frontier parity, AGI, consciousness, or autonomous authority claim follows from local conversation.",
        },
        {
            "schema": "NEXUS_DISCOVERY_CARD.v0.2.0",
            "discovery_id": "verification-as-the-gate-to-plasticity",
            "version": "0.1.0",
            "title": "Verification As The Gate To Plasticity",
            "status": "active",
            "summary": "Replay, paired benchmark comparison, retention, and authority invariants must pass before a learning proposal can request human authorization.",
            "math": {"law": "no paired improvement, no learning claim"},
            "code_references": ["nexus_gate/nex_core/verify.py"],
            "algorithm_card_refs": ["paired-learning-verification", "authority-invariant-verification"],
            "replication_steps": [".\\scripts\\nexus.ps1 nex-verify-all"],
            "evidence_surfaces": ["reports/nexus_cognitive_cycle_latest.json"],
            "next_versions": ["v0.2: add full held-out benchmark packs"],
            "boundary": "Verification can request authorization; it cannot grant it.",
        }
    ]
    return {
        "schema": SCHEMA,
