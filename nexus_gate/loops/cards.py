...
def build_loop_cards(root):
    ...
    reg = json.loads((root / "loops/nexus_loop_registry.v0.1.json").read_text(encoding="utf-8-sig"))
    cards = []
    for i, lid in enumerate(sorted(reg.get("loops", {})), 1):
        ...
        cards.append({
            ...
            "order": i,
            "loop_id": lid,
            "title": loop.get("title") or lid.replace("-", " ").title(),
            ...
            "command_surface": f'python -m nexus_gate.loops.runner --root . --loop {lid} --intent "<intent>" --json',
            "execute_surface": f'python -m nexus_gate.loops.runner --root . --loop {lid} --intent "<intent>" --execute --human-authorized --json',
            "powershell_surface": f'.\\scripts\\nexus.ps1 meta-loop -Loop {lid} -Tag "<intent>"',
            ...
            "authority_boundary": reg.get("authority_boundary", {}),
            "claim_boundary": reg.get("claim_boundary", ""),
...
def write_loop_cards(root):
    root = Path(root).resolve()
    pkt = build_loop_cards(root)
    (root / "state/loops").mkdir(parents=True, exist_ok=True)
    (root / "docs/runtime").mkdir(parents=True, exist_ok=True)
    enc = json.dumps(pkt, indent=2, sort_keys=True) + "\n"
    (root / "state/loops/nexus_loop_cards.v0.9.7.json").write_text(enc, encoding="utf-8")
    (root / "state/loops/nexus_loop_cards_latest.json").write_text(enc, encoding="utf-8")
    lines = [
        "# NEXUS Loop Cards",
        "",
        "NEXUS Loop Cards are HUD-ready JSON descriptions of the canonical local loop registry.",
        "v0.9.7 adds Toolbelt Cockpit card visibility and a separate JSON packet lane.",
        "",
    ...
    for c in pkt["cards"]:
        lines += [f"### {c['title']}", "", f"- Loop: `{c['loop_id']}`", f"- Group: `{c['hud']['display_group']}`", f"- Function: {c['function']}", f"- Command: `{c['command_surface']}`", f"- Execute: `{c['execute_surface']}`", ""]
...
def main(argv=None):
    ...
    pkt = write_loop_cards(Path(args.root))
    print(json.dumps(pkt, indent=2, sort_keys=True) if args.json else f"wrote {pkt['card_count']} NEXUS loop cards")
    return 0
