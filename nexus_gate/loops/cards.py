...
VERSION='0.9.4'; SCHEMA='NEXUS_LOOP_CARD.v0.9.4'
...
def build_loop_cards(root):
 ...
 return {'schema':'NEXUS_LOOP_CARD_SET.v0.9.4','version':VERSION,'system':'NEXUS_LOOP_CARDS','generated_for':'NEXUS_GATE_v0.9.4','source_registry':'loops/nexus_loop_registry.v0.1.json','generated_at':datetime.now(timezone.utc).isoformat(),'card_count':len(cards),'cards':cards,'authority_boundary':reg.get('authority_boundary',{}),'claim_boundary':'Loop cards are HUD-readable surfaces. They grant no authority.'}
def write_loop_cards(root):
 ...
 (root/'state/loops/nexus_loop_cards.v0.9.4.json').write_text(enc,encoding='utf-8'); (root/'state/loops/nexus_loop_cards_latest.json').write_text(enc,encoding='utf-8')
 lines=['# NEXUS Loop Cards','','NEXUS Loop Cards are HUD-ready JSON descriptions of the canonical local loop registry.','v0.9.4 expands cards into a personal coding paradise: creative planning, debug recovery, command palette, local oracle, docs weaving, continuity seal, and safe shipping loops.','','Boundary: loop cards describe governed local loops. They do not grant autonomous authority, execution authority, git write authority, memory promotion, safety proof, security proof, or correctness proof.','','## Card Surfaces','','- `state/loops/nexus_loop_cards.v0.9.4.json`','- `state/loops/nexus_loop_cards_latest.json`','- `python -m nexus_gate.loops.cards --root . --json`','- Spiral Core Portal option `[14] Nexus Loops / Cards`','','## Cards','']
