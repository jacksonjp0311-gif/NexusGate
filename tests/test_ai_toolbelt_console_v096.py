...
class TestAIToolbeltConsoleV096(unittest.TestCase):
 def test_registry_commands(self):
  r=json.loads((ROOT/"loops"/"nexus_loop_registry.v0.1.json").read_text(encoding="utf-8")); self.assertIn(r["generated_for"], {"NEXUS_GATE_v0.9.6", "NEXUS_GATE_v0.9.7"}); self.assertTrue(NEW.issubset(set(r["loops"])))
 ...
 def test_shells_and_docs(self):
  ...
  self.assertIn('"toolbelt"',ps); self.assertIn("function Invoke-NexusToolbelt",ps); self.assertIn("toolbelt|toolbelt-dashboard",sh); self.assertIn("AI Toolbelt Console",readme); self.assertTrue("Toolbelt Console Integration" in doc or "Toolbelt Cockpit Output" in readme); self.assertLess(len(readme.splitlines()),220)
