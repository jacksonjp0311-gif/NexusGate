(() => {
  ...
  function stationAnchor() {
    ...
    if (mode && mode.parentElement) return { parent: mode.parentElement, after: mode };

    const monitor = document.querySelector(".system-monitor-station");
    if (monitor && monitor.parentElement) return { parent: monitor.parentElement, after: monitor };

    const feedback = document.querySelector(".feedback-panel");
    if (feedback) return { parent: feedback, after: feedback.lastElementChild };

    const right = document.querySelector(".right-stack");
    if (right) return { parent: right, after: right.lastElementChild };
  ...
  function mountStation() {
    ...
    station.innerHTML =
      ...
      '</div>' +
      '<div class="gitnexus-core-note">Placed below Mode Selection. Run scripts/nexus_gitnexus.ps1 scan to refresh.</div>';
