(() => {
  ...
  const STATE_KEY = "__gitnexusGeometricMiniV037";
  ...
  function openBigHud() {
    if (typeof window.gitnexusHudMount === "function") {
      try { window.gitnexusHudMount(); } catch (_) {}
    }
  ...
  function removeDuplicateDocks() {
    const docks = qa("#" + DOCK_ID);
    docks.slice(1).forEach((node) => node.remove());

    const dock = document.getElementById(DOCK_ID);
    if (!dock) return;

    qa(".gitnexus-core-station, .gitnexus-mini-mirror-shell, .gitnexus-exact-mini-shell", dock).forEach((node) => node.remove());
  }
  ...
  function drawFrame(time) {
    ...
    const rnd = seededRandom(counts.files + counts.edges + 37);
