...
- Governed lanes: `status`, `evolve`, `nex-chat`, `nex-teach-begin`, `nex-teach-seal`, `nex-learn-propose`, `nex-verify-all`, `breath`, `telemetry-health`, `telemetry-fuse`, `conductance-field`, `conductance-replay-verify`, `ai-touch-begin`, `ai-touch-end`, `ai-touch-verify`, `intelligence-extract`, `intelligence-status`, `language-corpus-build`, `language-query`, `language-status`, `language-benchmark-smoke`, `language-replay-verify`, `self-model-build`, `epoch-seal`, `epoch-verify`, `epoch-chain-verify`, `action-recommend`, `action-authorize`, `action-execute`, `action-effects`, `action-final-evolve`, `action-validate`, `action-finalize`, `action-chain-verify`, `action-semantic-verify`, `experience-readiness`, `experience-seal`, `experience-chain-verify`, `calibration-status`, `calibration-authorize`, `calibration-apply`, `calibration-replay-verify`, `adaptive-coherence`, `emergence-report`, `first-learning-readiness`, `origin-seal`, `triadic-lattice`, `distill`, `decision-envelope`, `coherence-field`, `outcome-learn`, `runtime-hygiene`, `clean-runtime`, `install-hooks`, `reflect`, `domain`, `toolbelt`, `preflight`, `predictive-timing`, `predictive-evolve`, `predictive-memory`, `certificate-resume`, `cortex-refresh`, `wound-compress`, `phi-wound`, `phi-loop-auto`, and `phi-gate-auto`.
...
## Human Director Box
NEXUS GATE current line: v2.10.0 NEX Cognitive Cycle Engine, layered on v2.9.0 Geometric Language Intelligence Engine, v2.8.0 Cybernetic Field Engine, v2.7.1 Breath Pulse, v2.7.0 Governed Experience Engine, v2.6.2 Causal Action Receipt Loop, v2.6.1 Epoch Integrity Seal, v2.6.0 Evidence Distillation Graph, Causal Coherence Routing, and Triadic Geometric Lattice. v2.10.0 adds direct NEX CORE conversation through local NGLM, typed inner messages, immutable teaching episodes, bounded learning proposals, deterministic replay, and authority-invariant verification. Use `nex-chat`, `nex-mode-status`, `nex-teach-begin`, `nex-teach-seal`, `nex-learn-propose`, and `nex-verify-all`; NEX CORE can speak for verified system evidence, but cannot execute, authorize, mutate source, or apply persistent learning. Teaching is not learning; acceptance is not validation; no human authorization, no persistent plasticity.
## Causal Action Receipt Loop
v2.7.0 separates identity, experience, and plasticity. Recommendation, human authorization, registered execution, effects, action-bound final evolve, validation, learning, experience sealing, and calibration are separate surfaces under `state/actions/<action_id>/`, `state/experiences/<experience_id>/`, and hash-linked ledgers under `ledger/`. Effect proof is mandatory before validation, action-bound final evolve proof is mandatory before durable learning, registry definitions are hash-bound, expired authorizations block execution, and working-tree-only epochs cannot calibrate.
Use `action-recommend`, then explicit `action-authorize -ActionId "<id>"`, `action-execute`, `action-effects`, `action-final-evolve`, `action-validate`, `action-finalize`, and `experience-seal` only when a human chooses that route. Use `experience-readiness`, `action-semantic-verify`, `experience-chain-verify`, `adaptive-coherence`, and `emergence-report` to inspect readiness without execution. Calibration requires explicit `calibration-authorize` and remains bounded; v2.7.0 holds single-experience updates rather than automatically promoting route pressure. Generated visualization state, including the live neural repo graph, is runtime cache and cannot fabricate a new Source Epoch.
Use `.\scripts\nexus.ps1 breath` for semantic inhale/hold/exhale regulation. Use `telemetry-health` and `telemetry-fuse` for offline-safe observation context. Use `conductance-field` for sparse electrical-flow route preference. These are orientation surfaces only and cannot replace `evolve`.
...
`.\scripts\nexus.ps1 decision-envelope` selects a route; `.\scripts\nexus.ps1 coherence-field` scores field pressure; `.\scripts\nexus.ps1 outcome-learn` records route fitness and calibration. In v2.2 outcomes can adjust future recommendation pressure; they may not execute, self-authorize, mutate the repo, grant authority, or replace final `evolve`.
