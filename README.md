...
## Human Director Box
NEXUS GATE current line: v2.7.1 Breath Pulse, layered on v2.7.0 Governed Experience Engine, v2.6.3 Causal Integrity Hardening, v2.6.2 Causal Action Receipt Loop, v2.6.1a Epoch Integrity Hardening, Repo Neural Graph Runtime Hygiene, v2.6.1 Epoch Integrity Seal, v2.6.0 Evidence Distillation Graph, Triadic Geometric Lattice, and Causal Coherence Routing. Product identity is bound by Source Epochs, Observation Events, immutable `state/epochs/<source_epoch_id>/` packets, epoch ledgers, Origin Seal, Decision Envelope, Coherence Field, Triadic Lattice evidence, distilled graph nodes, registered command-route metadata, semantic action-chain verification, Experience Seals, explicit calibration receipts, and the read-only Breath Pulse. Older version strings are subsystem lineage until deliberate migration; authority remains human-bound.
...
Use `.\scripts\nexus.ps1 breath` for a compact inhale/hold/exhale vital-sign packet before broad work. Breath summarizes freshness, runtime pressure, Git scope, and the next bounded command; it is orientation evidence only and cannot replace `evolve`.
