...
- Governed lanes: `status`, `evolve`, `epoch-seal`, `epoch-verify`, `epoch-chain-verify`, `action-recommend`, `action-authorize`, `action-execute`, `action-effects`, `action-validate`, `action-finalize`, `action-chain-verify`, `first-learning-readiness`, `origin-seal`, `triadic-lattice`, `distill`, `decision-envelope`, `coherence-field`, `outcome-learn`, `runtime-hygiene`, `clean-runtime`, `install-hooks`, `reflect`, `domain`, `toolbelt`, `preflight`, `predictive-timing`, `predictive-evolve`, `predictive-memory`, `certificate-resume`, `cortex-refresh`, `wound-compress`, `phi-wound`, `phi-loop-auto`, and `phi-gate-auto`.
...
## Human Director Box
NEXUS GATE current line: v2.6.3 Causal Integrity Hardening, including v2.6.2 Causal Action Receipt Loop, v2.6.1a Epoch Integrity Hardening, Repo Neural Graph Runtime Hygiene, v2.6.1 Epoch Integrity Seal, v2.6.0 Evidence Distillation Graph, Triadic Geometric Lattice, and Causal Coherence Routing. Product identity is bound by Source Epochs, Observation Events, immutable `state/epochs/<source_epoch_id>/` packets, epoch ledgers, Origin Seal, Decision Envelope, Coherence Field, Triadic Lattice evidence, distilled graph nodes, and registered command-route metadata. Older version strings are subsystem lineage until deliberate migration; authority remains human-bound.
## Causal Action Receipt Loop
v2.6.3 hardens the law: **No receipt, no learning.** Recommendation, human authorization, registered execution, effects, validation, and learning are separate receipts under `state/actions/<action_id>/` and hash-linked ledgers under `ledger/`. Effect proof is mandatory before validation, final evolve proof is mandatory before durable learning, registry definitions are hash-bound, expired authorizations block execution, and working-tree-only epochs cannot calibrate.
Use `action-recommend`, then explicit `action-authorize -ActionId "<id>"`, `action-execute`, `action-effects`, `action-validate`, and `action-finalize` only when a human chooses that route. Use `first-learning-readiness` to see whether a clean admissible first learning run is ready. Generated visualization state, including the live neural repo graph, is runtime cache and cannot fabricate a new Source Epoch.
